...
const OrbitWorld = container => {
  ...
  var planet_object_list = [
...

    {
      ...
      source_url: "https://www.space.com/54-earth-history-composition-and-atmosphere.html",
    },
    {
      planet_code: 'mars',
      referencial_object_code: 'star_sun', // always a star for planet ? 
  ...
  var satellite_object_list = [
    {
      ...
      distance_to_ref: 0.356410, // in million of kms
      diameter: 3474.2,
    },
    {
      satellite_code: 'iss',
      referencial_object_code: 'planet_earh', // always a planet for a satellite ? 
      distance_to_ref: 0.000400, // in million of kms // 356 410 km au périgée et 405 500 km à l'apogée.
      speed: 28000, // in km/h
      diameter: 0.3,
  ...
  // PLANET INFORMATION
  // Set PlanetDistance
  var distance_sun_earth = planet_object_list.find(obj => obj.planet_code === 'earth').distance_to_ref / 10 // scale : divide by 10
  var distance_sun_mars = planet_object_list.find(obj => obj.planet_code === 'mars').distance_to_ref / 10 // scale : divide by 10
  // 
  let theta_earth = 0
  let theta_mars = 0
  let theta_philae = 0
  ...
  const rotation_speed_earth = (2 * Math.PI) / planet_object_list.find(obj => obj.planet_code === 'earth').orbit_revolution
  const rotation_speed_mars = (2 * Math.PI) / planet_object_list.find(obj => obj.planet_code === 'mars').orbit_revolution
  // Set satellites rotation speeds (is approximation because some are non-linear)
  const rotation_speed_philae = (2 * Math.PI) / satellite_object_list.find(obj => obj.satellite_code === 'philae').orbit_revolution_length
  // 
  // 
  // Orbits
